...
info[cs.REPORT.VEHICLE] = new Telegram()
  ...
  .uint8("chargeStatus", {
    formatter: toEnum1(...Object.keys(cs.CHARGE_STATUS)),
  }) // 充电状态
...
info[cs.REPORT.CUSTOM_EXT] = new Telegram()
  ...
  .uint16("dataLen", {
    formatter: val => {
      if (val !== 0x30)
        throw new Error(`custome ext section has wrong lenth ${val}`);
      return val;
    },
  }) // 数据长度
  ...
  .uint32("totalCharge", {
    formatter: toNumber(0.1, 0, 0xfffffffe, 0xffffffff),
  }) // 累积充电电量
  .uint32("totalDischarge", {
    formatter: toNumber(0.1, 0, 0xfffffffe, 0xffffffff),
  }) // 累积放电电量
  ...
  .bit2("middleDoorStatus", {
    formatter: toEnum0(...Object.keys(cs.DOOR_STATUS)),
  }) // 中门
  .bit2("frontDoorStatus", {
    formatter: toEnum0(...Object.keys(cs.DOOR_STATUS)),
  }) // 前门
  .bit2("handbrakeStatus", {
    formatter: toEnum0(...Object.keys(cs.HANDBRAKE_STATUS)),
  }) // 手刹
...
info[cs.REPORT.TEN_SECONDS] = new Telegram().endianess("big").array("datas", {
  ...
  type: new Telegram()
    ...
    .skip(1)
    .uint16("totalCurrent", {
      formatter: toNumber(0.1, -1000, 0xfffe, 0xffff),
    }), // 总电流
});
...
info[cs.REPORT.ADAS] = new Telegram().endianess("big").array("datas", {
  ...
  type: new Telegram()
    ...
    .skip(1)
    .uint8("obstacleType", {
      formatter: toEnum0(...Object.keys(cs.OBSTACLE_TYPE)),
    }) // 障碍物类型
    .skip(1)
